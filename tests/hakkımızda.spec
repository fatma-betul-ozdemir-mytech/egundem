test.describe('eGündem Test Suite - Hakkımızda Sayfası', () => {
  ...
  // EGT-45: Hakkımızda sayfası başarıyla yüklenmeli
  test('EGT-45 - Hakkımızda sayfası yüklenmeli', async ({ page }) => {
  ...
  // EGT-46: Sayfada şirket hakkında bilgilendirici içerik olmalı
  test('EGT-46 - Şirket hakkında içerik görüntülenmeli', async ({ page }) => {
  ...
  // EGT-47: Sayfa içindeki görseller doğru şekilde yüklenmeli
  test('EGT-47 - Görseller görünür olmalı ve yüklenmeli', async ({ page }) => {
    await page.goto(`${BASE_URL}/hakkimizda`);
  ...
  // EGT-48: Sayfada ekip üyeleri veya yönetim kadrosu bilgileri olmalı
  test('EGT-48 - Ekip üyeleri veya yönetim kadrosu listelenmeli', async ({ page }) => {
    await page.goto(`${BASE_URL}/hakkimizda`);
    const ekipBaslik = page.locator('text=ekip|yönetim|takım|personel');
